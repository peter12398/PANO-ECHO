...
class CEEAudioLayer(nn.Module):
    def __init__(self, channels, SE=True):
        super(CEEAudioLayer, self).__init__()

        self.res_conv1 = nn.Conv2d(channels * 3, channels, kernel_size=1, padding=0, bias=False)
        self.res_bn1 = nn.BatchNorm2d(channels)
    ...
    def forward(self, equi_feat, c2e_feat, audio_feat):

        x = torch.cat([equi_feat, c2e_feat, audio_feat], 1)
        x = self.relu(self.res_bn1(self.res_conv1(x)))
        shortcut = self.res_bn2(self.res_conv2(x))
